...
where
  C: Service<Uri> + Clone + CheckDst + Send + 'static,
  C::Response:
  ...
  fn call(&mut self, orig_dst: Uri) -> Self::Future {
    ...
    if let Some(intercept) = self.intercept(&orig_dst).cloned() {
      ...
      let proxying: Self::Future = match intercept.target {
            ...
            let (dst, ech_mode) =
              match lookup_https(&https_resolver, &orig_dst).await? {
                Some(https) => {
                  ...
                  let host = https.host;
                  let port = https.port;
                  dst = format!("http://{host}:{port}").parse().unwrap();
          ...
          Box::pin(async move {
            ...
            let (dst, ech_mode) =
              match lookup_https(&https_resolver, &orig_dst).await? {
                Some(https) => {
                  is_https = true;
                  dst = https.host.to_string();
                  let host = dst.as_str();
                  let port = https.port;
                  ((host, port), https.ech_config.map(Into::into))
                }
                None => {
                  let port = orig_dst.port_u16().unwrap_or(if is_https {
                    443
                  } else {
                    80
                  });
            ...
            let io = if let Some((user, pass)) = auth {
              Socks5Stream::connect_with_password(socks_addr, dst, &user, &pass)
                .await?
            } else {
              Socks5Stream::connect(socks_addr, dst).await?
            };
        ...
        Target::Tcp {
          ...
          let connecting = http.call(uri);
          Box::pin(async move {
            let io = connecting.await.map_err(Into::into)?;
            Ok(Proxied::Tcp(MaybeHttpsStream::Http(io)))
          })
        }
        #[cfg(not(windows))]
        Target::Unix { path } => {
    ...
    Box::pin(async move {
      ...
      if is_https {
        ...
        let tokio_io = TokioIo::new(io);
        let io = TlsConnector::from(tls)
          .connect(TryFrom::try_from(host.to_owned())?, tokio_io)
          .await?;
        let tokio_io = TokioIo::new(io);
        Ok(Proxied::PassThrough(MaybeHttpsStream::Https(tokio_io)))
      } else {
        Ok(Proxied::PassThrough(MaybeHttpsStream::Http(io)))
      }
    })
  }
}
